...
class TestFromGatekeeperResult:
    ...
    def test_execute_result(self):
        ...
        est = to_certificate_estimate(R=0.7, S_sup=0.2, N=0.1, kappa_compat=0.8, sigma=0.2, alpha=0.875)
    ...
    def test_high_noise_rejects(self):
        gk = SequentialGatekeeper()
        est = to_certificate_estimate(R=0.1, S_sup=0.1, N=0.8, kappa_compat=0.3, sigma=0.8, alpha=0.11)
        result = gk.evaluate(est)
        decision = from_gatekeeper_result(result)
        assert decision.allowed is False


class TestToCPGatekeeperInput:
    """Tests for to_certificate_estimate()."""

    def test_basic_estimate(self):
        est = to_certificate_estimate(R=0.6, S_sup=0.3, N=0.1, kappa_compat=0.7, sigma=0.3, alpha=0.85)
    ...
    def test_alpha_auto_computed(self):
        est = to_certificate_estimate(R=0.6, S_sup=0.3, N=0.1, kappa_compat=0.7, sigma=0.3)
        expected_alpha = 0.6 / (0.6 + 0.1)
        assert abs(est.alpha - expected_alpha) < 1e-9

    def test_alpha_zero_division(self):
        est = to_certificate_estimate(R=0.0, S_sup=1.0, N=0.0, kappa_compat=0.5, sigma=0.5)
...
class TestThresholdsToConfig:
    ...
    def test_basic_mapping(self):
        config = thresholds_to_config({"kappa_threshold": 0.7, "R": 0.3, "S_sup": 0.4, "N": 0.5})
    ...
    def test_config_accepted_by_gatekeeper(self):
        ...
        est = to_certificate_estimate(R=0.5, S_sup=0.3, N=0.2, kappa_compat=0.6, sigma=0.3, alpha=0.71)
        result = gk.evaluate(est)
        assert isinstance(result, GatekeeperResult)
